import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
import math
import os
from scipy.spatial.distance import euclidean
from fastdtw import fastdtw # Import fastdtw
import seaborn as sns # For setting style

# --- Configuration ---
# !!! Path to the folder containing 1.csv, 2.csv, etc. !!!
PARENT_FOLDER_PATH = '/home/kadi/Desktop/Thesis/demo_processor_new/data/transformed/button'

# Option 1: Load all CSV files in the folder
LOAD_ALL_FILES = True
# Option 2: Specify a list of filenames (if LOAD_ALL_FILES is False)
# FILE_LIST = ['1.csv', '3.csv', '5.csv']
FILE_LIST = [] # Ignored if LOAD_ALL_FILES is True

# ***** NEW: Select Alignment Type *****
ALIGNMENT_TYPE = 'DDTW' # Options: 'DTW', 'DDTW'

# Features to use for Alignment distance calculation (DTW or DDTW)
ALIGNMENT_FEATURE_COLS = ['tx', 'ty', 'tz']

print(f"Using features for {ALIGNMENT_TYPE}: {ALIGNMENT_FEATURE_COLS}")

# --- Data Loading ---

def load_selected_data(parent_folder, load_all=True, file_list=None, feature_columns=None):
    """Loads specified trajectories and features from CSV files."""
    all_trajectories = []
    loaded_filenames = []

    if feature_columns is None:
        feature_columns = ['tx', 'ty', 'tz'] # Default if not provided

    if load_all:
        try:
            # List only .csv files
            all_files_in_dir = [f for f in os.listdir(parent_folder) if os.path.isfile(os.path.join(parent_folder, f))]
            filenames = [f for f in all_files_in_dir if f.endswith('.csv')]
            # Optional: Sort filenames numerically if they represent sequence numbers
            try:
                 filenames.sort(key=lambda x: int(os.path.splitext(x)[0]))
            except ValueError:
                 filenames.sort() # Sort alphabetically if not purely numeric
            print(f"Found {len(filenames)} CSV files to load: {filenames}")
            if not filenames:
                 print(f"Warning: No CSV files found in {parent_folder}")
                 return [], []
        except FileNotFoundError:
            print(f"Error: Parent folder not found - {parent_folder}")
            return [], []
    elif file_list:
        filenames = file_list
        print(f"Loading specified files: {filenames}")
    else:
        print("Error: No files specified to load.")
        return [], []

    for filename in filenames:
        file_path = os.path.join(parent_folder, filename)
        # print(f"  Loading {file_path}...") # Reduce verbosity
        try:
            df = pd.read_csv(file_path)
            # Check if all required columns exist
            missing_cols = [col for col in feature_columns if col not in df.columns]
            if missing_cols:
                raise KeyError(f"Missing columns: {missing_cols}")

            # Select only the specified feature columns for alignment
            trajectory_data = df[feature_columns].values
            if trajectory_data.shape[1] != len(feature_columns):
                 raise ValueError(f"Expected {len(feature_columns)} feature columns, found {trajectory_data.shape[1]}")

            # Basic NaN check (consider more robust handling if needed)
            if np.isnan(trajectory_data).any():
                print(f"Warning: NaN values found in {file_path}. Skipping file.")
                continue
            if trajectory_data.shape[0] == 0:
                 print(f"Warning: Trajectory data is empty in {file_path}. Skipping file.")
                 continue

            all_trajectories.append(trajectory_data)
            loaded_filenames.append(filename)

        except FileNotFoundError:
            print(f"Warning: File not found - {file_path}")
        except KeyError as e:
            print(f"Error: Missing expected column in {file_path}: {e}")
            # print(f"  Expected columns: {feature_columns}") # Already printed
        except Exception as e:
            print(f"Error loading or processing {file_path}: {e}")

    if not all_trajectories:
         print("Warning: No valid trajectory data loaded.")

    return all_trajectories, loaded_filenames

# --- Alignment (DTW/DDTW) ---

def calculate_derivative(trajectory):
    """Estimates derivative using simple finite difference."""
    if len(trajectory) < 2:
        return np.zeros_like(trajectory) # Return zeros if too short
    # diff results in length N-1, pad at the beginning to keep length N
    derivative = np.diff(trajectory, axis=0, prepend=trajectory[0:1,:])
    return derivative

# ***** MODIFIED: Function renamed and handles DTW/DDTW *****
def align_trajectories(trajectories, alignment_type):
    """
    Aligns all trajectories to the longest one using DTW or DDTW.

    Args:
        trajectories (list): List of numpy arrays (n_points, n_features).
                             Features used are defined by ALIGNMENT_FEATURE_COLS.
        alignment_type (str): 'DTW' or 'DDTW'.

    Returns:
        tuple: (aligned_trajectories, reference_trajectory, reference_index)
               aligned_trajectories: List of warped trajectories.
               reference_trajectory: The reference trajectory.
               reference_index: Index of the reference trajectory.
    """
    if not trajectories: return [], None, -1
    if alignment_type not in ['DTW', 'DDTW']:
        raise ValueError("alignment_type must be 'DTW' or 'DDTW'")

    lengths = [len(t) for t in trajectories]
    if not lengths: return [], None, -1
    reference_index = np.argmax(lengths)
    reference_trajectory = trajectories[reference_index] # This is the one with features used for alignment
    ref_len = len(reference_trajectory)
    print(f"\nUsing trajectory {reference_index} (length {ref_len}) as reference for {alignment_type}.")

    aligned_trajectories = []
    for i, traj in enumerate(trajectories): # traj contains only alignment features
        print(f"  Aligning trajectory {i} (length {len(traj)}) to reference...")
        if len(traj) == 0:
             print(f"    Skipping empty trajectory {i}")
             aligned_trajectories.append(np.zeros_like(reference_trajectory))
             continue
        if i == reference_index:
            aligned_trajectories.append(reference_trajectory.copy())
            continue

        # --- Select data for fastdtw based on alignment_type ---
        if alignment_type == 'DTW':
            ref_data_for_dtw = reference_trajectory
            traj_data_for_dtw = traj
            # print(f"    Using DTW...") # Redundant with outer print
        elif alignment_type == 'DDTW':
            ref_data_for_dtw = calculate_derivative(reference_trajectory)
            traj_data_for_dtw = calculate_derivative(traj)
            # print(f"    Using DDTW (derivatives)...") # Redundant
            if not np.any(ref_data_for_dtw) and not np.any(traj_data_for_dtw):
                 print("    Warning: Both derivative sequences are zero. Using standard DTW as fallback.")
                 ref_data_for_dtw = reference_trajectory
                 traj_data_for_dtw = traj

        # Perform fastdtw
        distance, path = fastdtw(ref_data_for_dtw, traj_data_for_dtw, dist=euclidean)
        print(f"    Alignment distance ({alignment_type}): {distance:.4f}")

        # --- Warp the ORIGINAL trajectory features using the obtained path ---
        path_array = np.array(path)
        warped_traj = np.zeros_like(reference_trajectory) # Initialize with ref shape
        ref_indices_in_path = path_array[:, 0]; traj_indices_in_path = path_array[:, 1]

        for ref_idx in range(ref_len):
            matching_path_indices = np.where(ref_indices_in_path == ref_idx)[0]
            if len(matching_path_indices) > 0:
                traj_idx = traj_indices_in_path[matching_path_indices[-1]]
                traj_idx = min(traj_idx, len(traj) - 1)
                warped_traj[ref_idx] = traj[traj_idx] # Warp the original feature data
            else:
                # print(f"Warning: No matching path point found for ref idx {ref_idx}. Repeating previous.") # Reduce noise
                if ref_idx > 0: warped_traj[ref_idx] = warped_traj[ref_idx - 1]
                elif len(traj) > 0: warped_traj[ref_idx] = traj[0]
        aligned_trajectories.append(warped_traj)

    return aligned_trajectories, reference_trajectory, reference_index

# --- Visualization ---

def plot_all_original_trajectories(original_trajectories, feature_cols):
    """Plots X, Y, Z components of all original trajectories on the same axes in black."""
    n_traj = len(original_trajectories)
    if n_traj == 0: print("No valid original trajectories to plot."); return
    original_trajectories = [t for t in original_trajectories if len(t)>0]
    if not original_trajectories: print("Not enough valid trajectories to plot originals."); return

    n_features = original_trajectories[0].shape[1]
    n_plot_dims = min(n_features, 3) # Plot up to 3 dimensions
    dim_labels = feature_cols[:n_plot_dims] if len(feature_cols) >= n_plot_dims else [f'Dim {i}' for i in range(n_plot_dims)]

    fig, axs = plt.subplots(n_plot_dims, 1, figsize=(12, 4 * n_plot_dims), sharex=False)
    if n_plot_dims == 1: axs = [axs]
    fig.suptitle('All Original Trajectories (Overlaid)', fontsize=16)
    sns.set_style("whitegrid")

    for i in range(len(original_trajectories)): # Use length after filtering
        traj = original_trajectories[i]
        time_steps = np.arange(len(traj))
        for j in range(n_plot_dims):
            axs[j].plot(time_steps, traj[:, j], color='black', alpha=0.5, linewidth=1)
            axs[j].set_ylabel(dim_labels[j])
            if j == n_plot_dims - 1: axs[j].set_xlabel('Time Step (Original)')
            axs[j].grid(True, linestyle=':')
    plt.tight_layout(rect=[0, 0.03, 1, 0.96]); plt.show()

def plot_all_aligned_trajectories(aligned_trajectories, feature_cols, alignment_type):
    """Plots X, Y, Z components of all aligned trajectories on the same axes in black."""
    n_traj = len(aligned_trajectories)
    if n_traj == 0: print("No valid aligned trajectories to plot."); return
    aligned_trajectories = [t for t in aligned_trajectories if len(t)>0]
    if not aligned_trajectories: print("Not enough valid trajectories to plot aligned."); return

    n_features = aligned_trajectories[0].shape[1]
    n_plot_dims = min(n_features, 3)
    ref_len = len(aligned_trajectories[0])
    aligned_time = np.arange(ref_len)
    dim_labels = feature_cols[:n_plot_dims] if len(feature_cols) >= n_plot_dims else [f'Dim {i}' for i in range(n_plot_dims)]

    fig, axs = plt.subplots(n_plot_dims, 1, figsize=(12, 4 * n_plot_dims), sharex=True)
    if n_plot_dims == 1: axs = [axs]
    fig.suptitle(f'All {alignment_type}-Aligned Trajectories (Overlaid)', fontsize=16) # Use alignment type in title
    sns.set_style("whitegrid")

    for i in range(len(aligned_trajectories)): # Use length after filtering
        traj = aligned_trajectories[i]
        if len(traj) != ref_len: continue # Skip length mismatches
        for j in range(n_plot_dims):
            axs[j].plot(aligned_time, traj[:, j], color='black', alpha=0.5, linewidth=1)
            axs[j].set_ylabel(dim_labels[j])
            if j == n_plot_dims - 1: axs[j].set_xlabel(f'Time Step ({alignment_type} Aligned)')
            axs[j].grid(True, linestyle=':')
    plt.tight_layout(rect=[0, 0.03, 1, 0.96]); plt.show()


# --- Main Execution ---
if __name__ == "__main__":
    # 1. Load data using only the features needed for the selected alignment
    original_trajs, loaded_files = load_selected_data(
        PARENT_FOLDER_PATH,
        load_all=LOAD_ALL_FILES,
        file_list=FILE_LIST,
        feature_columns=ALIGNMENT_FEATURE_COLS # Load only features needed for alignment
    )

    if not original_trajs:
        print("Exiting: No trajectories loaded.")
        exit()

    # 2. Plot all original trajectories together
    print("\nPlotting all original trajectories...")
    plot_all_original_trajectories(original_trajs, ALIGNMENT_FEATURE_COLS)

    # 3. Perform Alignment (DTW or DDTW)
    # ***** MODIFIED: Call generic alignment function *****
    aligned_trajs, ref_traj, ref_idx = align_trajectories(original_trajs, ALIGNMENT_TYPE)

    # 4. Plot all aligned trajectories together
    if aligned_trajs:
        print("\nPlotting all aligned trajectories...")
        plot_all_aligned_trajectories(aligned_trajs, ALIGNMENT_FEATURE_COLS, ALIGNMENT_TYPE)
    else:
        print("Alignment failed or produced no valid trajectories, cannot plot aligned results.")

    # --- Ready for downstream processing ---
    valid_aligned_trajs = [t for t in aligned_trajs if len(t) > 0 and np.any(t)]
    if valid_aligned_trajs:
        print(f"\nVariable 'valid_aligned_trajs' contains {len(valid_aligned_trajs)} non-empty warped trajectories.")
        # These trajectories contain only the ALIGNMENT_FEATURE_COLS.
        # If downstream tasks (like HMM, CPD, ProMP) need different or more features,
        # the alignment function needs to be modified to warp the *full* data based on the
        # path derived from the *alignment* features (as done in previous HMM/CPD examples).
        # Or, reload the full data after getting the alignment path.
    else:
        print("\nNo valid aligned trajectories available.")

